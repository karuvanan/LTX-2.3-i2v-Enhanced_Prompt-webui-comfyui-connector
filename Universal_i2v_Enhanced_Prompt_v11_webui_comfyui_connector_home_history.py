...
class LTX23PromptGUI:
    ...
    def _comfy_jobs_path(self):
        """
        ✅ Persistent Job History 文件路径。
        WebUI refresh 后仍可查询已发送的 ComfyUI job。
        """
        import os
        runtime_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "runtime")
        os.makedirs(runtime_dir, exist_ok=True)
        return os.path.join(runtime_dir, "comfyui_jobs.json")

    def _load_comfy_jobs(self):
        """✅ 读取 ComfyUI job history。"""
        import json
        import os

        path = self._comfy_jobs_path()
    ...
    def _save_comfy_jobs(self, jobs):
        """✅ 保存 ComfyUI job history。"""
        import json
    ...
    def _upsert_comfy_job(self, job_data):
        """
        ✅ 新增或更新单个 ComfyUI job。
        用 prompt_id 作为唯一 key。
        """
        import threading

        if not hasattr(self, "_comfy_jobs_lock"):
            self._comfy_jobs_lock = threading.Lock()

        prompt_id = str(job_data.get("prompt_id", "")).strip()
        if not prompt_id:
            return

        with self._comfy_jobs_lock:
            jobs = self._load_comfy_jobs()
            found = False

            for i, job in enumerate(jobs):
                if str(job.get("prompt_id", "")) == prompt_id:
                    old = dict(job)
                    old.update(job_data)
                    jobs[i] = old
                    found = True
            ...
            # 只保留最近 50 个，避免 history 无限变大
            jobs = jobs[:50]
            self._save_comfy_jobs(jobs)

    def _format_comfy_jobs_dataframe(self):
        """✅ 将 jobs 转成 Gradio Dataframe 可显示格式。"""
        import os
        try:
            import pandas as pd
        except Exception:
        ...
        for job in jobs:
            ...
            rows.append({
                ...
                "seed": job.get("seed", ""),
                "downloaded_file": f,
                "file_exists": "yes" if f and os.path.exists(f) else "",
                "message": job.get("message", ""),
            })

        columns = [
            "status", "prompt_id", "started_at", "completed_at",
            "duration_seconds", "seed", "downloaded_file", "file_exists", "message"
        ]
    ...
    def web_refresh_comfy_job_history(self):
        """
        ✅ WebUI: refresh 后也可以重新读取 job history。
        返回：table, latest_download_file, status_text
        """
        import os

        jobs = self._load_comfy_jobs()
        table = self._format_comfy_jobs_dataframe()
        ...
        if not jobs:
            return table, None, "No ComfyUI job history yet."

        running_count = sum(1 for j in jobs if j.get("status") in ("queued", "running"))
        ...
        status = (
            ...
            f"Running/Queued: {running_count}\n"
            f"Completed: {completed_count}\n"
            f"Failed: {failed_count}\n"
            "如果刚刚 refresh 了 WebUI，只要点击 Refresh Job History 即可恢复状态。"
        )
        return table, latest_file, status

    def web_query_comfy_job_by_prompt_id(self, prompt_id):
        """✅ WebUI: 根据 prompt_id 查询某个 job，并返回可下载文件。"""
        import os

        prompt_id = str(prompt_id or "").strip()
        if not prompt_id:
            return "请输入 prompt_id。", None

        jobs = self._load_comfy_jobs()
        for job in jobs:
            if str(job.get("prompt_id", "")) == prompt_id:
                f = job.get("downloaded_file") or ""
                text = (
                    f"Status: {job.get('status', '')}\n"
                    f"Prompt ID: {job.get('prompt_id', '')}\n"
                    f"Started At: {job.get('started_at', '')}\n"
                    f"Completed At: {job.get('completed_at', '')}\n"
                    f"Duration: {job.get('duration_seconds', '')}\n"
                    f"Seed: {job.get('seed', '')}\n"
                    f"File: {f}\n"
                    f"File Exists: {'yes' if f and os.path.exists(f) else 'no'}\n"
                    f"Message: {job.get('message', '')}"
                )
                return text, f if f and os.path.exists(f) else None

        return f"No local job found for prompt_id: {prompt_id}", None

    def web_clear_finished_comfy_job_history(self):
        """✅ 清理 completed / failed，只保留 queued / running。"""
        jobs = self._load_comfy_jobs()
        kept = [j for j in jobs if j.get("status") in ("queued", "running")]
        removed = len(jobs) - len(kept)
        self._save_comfy_jobs(kept)
        table = self._format_comfy_jobs_dataframe()
        return table, None, f"已清理 {removed} 个 finished job，只保留 running / queued job。"

    def _background_wait_download_comfy_job(
        self,
        server_url,
        prompt_id,
        timeout_seconds,
        ...
        """
        ✅ 后台等待 ComfyUI 完成并下载 SaveVideo。
        这个 thread 不依赖当前 Gradio 页面；手机 refresh 后仍然继续跑。
        """
        import datetime

        try:
            self._upsert_comfy_job({
                "status": "running",
                "prompt_id": prompt_id,
                "duration_seconds": int(duration_seconds or 0),
                "seed": int(final_seed or 0),
                "message": "Background worker is waiting for ComfyUI SaveVideo output...",
            })
            ...
            self._upsert_comfy_job({
                ...
                "downloaded_file": local_file or "",
                "message": (
                    "✅ SaveVideo completed and downloaded. "
                    f"Node={media_info.get('node_id')}, "
                    f"filename={media_info.get('filename')}"
                ),
            })
            ...
            self._upsert_comfy_job({
                ...
                "prompt_id": prompt_id,
                "completed_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "message": f"❌ Background wait/download failed: {e}",
            })
    ...
    def comfy_send_ltx23_i2v_to_queue(
        ...
        """
        ✅ WebUI 一键连接 ComfyUI：上传图片 → 改 workflow 节点 → Queue。
        ✅ 新版：不再让 Gradio 页面一直等待下载；改成后台 thread 等待 SaveVideo。
        ✅ refresh WebUI 后不会丢任务，可在 Job History 查询 / 下载。
        ...
        try:
            ...
            self._upsert_comfy_job({
                "status": "queued" if wait_for_output else "submitted",
                "prompt_id": prompt_id,
                ...
                "seed": int(final_seed or 0),
                "downloaded_file": "",
                "message": "Sent to ComfyUI queue. Waiting worker will continue in background." if wait_for_output else "Sent to ComfyUI queue without background waiting.",
            })
            ...
            if wait_for_output:
                timeout = int(output_timeout_seconds or 1800)
                t = threading.Thread(
                    target=self._background_wait_download_comfy_job,
                    args=(
                        server_url,
                        prompt_id,
                        timeout,
                ...
                status += (
                    "\n✅ 后台等待已经启动。\n"
                    "你现在可以 refresh WebUI、关闭手机页面、或稍后回来。\n"
                    "ComfyUI 会继续在背后生成，Python 后台会继续等待 SaveVideo。\n"
                    "完成后请点击 STEP 6 的 Refresh Job History 查看并下载。"
                )
            else:
                status += "\nℹ️ 已加入 Queue，但没有开启后台等待。请到 ComfyUI output 文件夹查看。"
    ...
    def start_gradio_server(self):
        ...
        with gr.Blocks(title="LTX-2.3 手机端独立控制台") as web_ui:
            gr.Markdown("# 📱 LTX-2.3 Sulphur Prompt Generator + ComfyUI Job History")
            gr.Markdown("🟢 打开网页即可看到 Load Image 与 Job History｜Top 5 流派 → 自动动作方案 → ComfyUI Queue → History 下载")
            ...
            # ─── HOME QUICK HISTORY：打开网页即可看到 Load Image + Job History ───
            gr.Markdown("---")
            gr.Markdown("### 📜 ComfyUI Job History（打开网页即可查看 / refresh 后可恢复）")
            gr.Markdown("手机不小心 refresh 没关系：ComfyUI 会继续跑。回来后直接点 Refresh Job History，就能看到 running / completed / failed，并下载最终 SaveVideo。")

            with gr.Row():
                home_refresh_history_btn = gr.Button("🔄 Refresh Job History", variant="secondary")
                home_clear_finished_history_btn = gr.Button("🧹 Clear Finished History", variant="secondary")

            home_job_history_table = gr.Dataframe(
                label="ComfyUI Job History",
                interactive=False,
                wrap=True
            )

            with gr.Row():
                home_query_prompt_id_input = gr.Textbox(
                    label="Query Prompt ID",
                    placeholder="Paste prompt_id here"
                )
                home_query_job_btn = gr.Button("🔍 Query Job", variant="secondary")

            home_query_job_output = gr.Textbox(
                label="Query Result",
                lines=6
            )

            home_history_video_file = gr.File(
                label="⬇️ Download Completed SaveVideo from History",
                interactive=False
            )

            home_history_status = gr.Textbox(
                label="History Status",
                value="打开网页后可以直接点击 Refresh Job History 查看任务。",
                lines=4,
                interactive=False
            )
            ...
            with gr.Column(visible=False) as hidden_panel:
                ...
                with gr.Row():
                    comfy_wait_output = gr.Checkbox(
                        label="后台自动抓取 Save Video，refresh 后可在 History 下载",
                        value=True
                    )
                ...
                comfy_video_file = gr.File(
                    label="⬇️ Save Video 下载（当前页面即时返回；新版主要用下方 History 下载）",
                    interactive=False
                )

                gr.Markdown("---")
                gr.Markdown("### 📜 STEP 6: ComfyUI Job History / Query / Download")
                gr.Markdown("如果手机不小心 refresh，ComfyUI 仍会继续跑。回来后点击 Refresh Job History，就可以看到 running / completed / failed，并下载最终 SaveVideo。")

                with gr.Row():
                    refresh_history_btn = gr.Button("🔄 Refresh Job History", variant="secondary")
                    clear_finished_history_btn = gr.Button("🧹 Clear Finished History", variant="secondary")

                job_history_table = gr.Dataframe(
                    label="ComfyUI Job History",
                    interactive=False,
                    wrap=True
                )
                ...
                with gr.Row():
                    query_prompt_id_input = gr.Textbox(
                        label="Query Prompt ID",
                        placeholder="Paste prompt_id here"
                    )
                    query_job_btn = gr.Button("🔍 Query Job", variant="secondary")

                query_job_output = gr.Textbox(
                    label="Query Result",
                    lines=8
                )

                history_video_file = gr.File(
                    label="⬇️ Download Completed SaveVideo from History",
                    interactive=False
                )
            ...
            # ✅ 首页可见 Job History：打开 WebUI 即可 refresh / query / download
            home_refresh_history_btn.click(
                fn=self.web_refresh_comfy_job_history,
                inputs=[],
                outputs=[home_job_history_table, home_history_video_file, home_history_status]
            )

            home_query_job_btn.click(
                fn=self.web_query_comfy_job_by_prompt_id,
                inputs=[home_query_prompt_id_input],
                outputs=[home_query_job_output, home_history_video_file]
            )

            home_clear_finished_history_btn.click(
                fn=self.web_clear_finished_comfy_job_history,
                inputs=[],
                outputs=[home_job_history_table, home_history_video_file, home_history_status]
            )

            # ✅ Persistent Job History：refresh 后重新查询 job / 下载最终视频
            refresh_history_btn.click(
                fn=self.web_refresh_comfy_job_history,
                inputs=[],
                outputs=[job_history_table, history_video_file, comfy_status]
            )

            query_job_btn.click(
                fn=self.web_query_comfy_job_by_prompt_id,
                inputs=[query_prompt_id_input],
                outputs=[query_job_output, history_video_file]
            )

            clear_finished_history_btn.click(
                fn=self.web_clear_finished_comfy_job_history,
                inputs=[],
                outputs=[job_history_table, history_video_file, comfy_status]
            )
